...
Por ello, antes de comenzar la paralelización, se han realizado ciertas modificaciones de diseño. 
...
La estructura del BVH consiste en una jerarquía de volúmenes que agrupan los elementos de la escena, tratándolos como un árbol binario en el que cada 
caja ya definida en AABB se incluye en otra caja mayor que engloba varias de ellas, de este modo, podemos descartar el cálculos de intersección de grandes cantidades de objetos, pues si no hay intersección con la caja mayor, no es necesario comprobar las cajas interiores. 
...
Por su parte, el renderizado ha sufrido modificaciones más agresivas, casi reescribiendo el funcionamiento estructural del módulo. Con la implementación del BVH se ha tenido que re implementar el renderizado bajo la detección de colisiones entre rayos y objetos.
...
En este caso, definimos las semillas de cada hilo como atómicas, haciendo que no se repitan entre hilos y no generen ningún tipo de secuencia o patrón.
